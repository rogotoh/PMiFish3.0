...
#option setting
my ($file, $primer, $diff, $out, $separate, $log, $shift, $length);
for (my $v = 0; $v < @ARGV; $v++){
	...
	elsif($ARGV[$v] =~ /-shift$/){$shift = $ARGV[$v+1];}
	elsif($ARGV[$v] =~ /-length$/){$length = 1;}
}
...
unless($length){
	...
	unless($shift =~ /\d+/){print "-shift option error!\n"; exit;}
}

#shift setting
#unless($shift){$shift = 2;}
...
my $count = 0;
my (%forward, %reverse, $name, @primername, $f_len, $r_len);
while(<DATA2>){
	if($_ =~ /\#/){next;}
	$_ =~ s/\r//g;
	chomp($_);
	...
	if($count == 1){
		...
		elsif($_ =~ /^[a-z]/i){$_ =~ tr/[a-z]/[A-Z]/; $forward{$name} = $_; $f_len = length($_);}
	}
	if($count == 2){
		if($_ =~ /^>(.+)/){$name = $1;
		}elsif($_ =~ /^[a-z]/i){
	...
	unless($count == 3){push(@data, $_); $count++;}
		...
		foreach(@primername){
			...
			if($data[1] =~ /^(\w*)$fcut{$_}(\w+)$rcut{$_}(\w*)$/){
				...
				my $posi = $fn + length($fnc{$_});
				my $templ = length($fnc{$_});
	...
	if($filenum > 1){
		...
		foreach(@primername){
			if($unlink{$_}){
				...
				open (LOG, ">", "${out}_${_}_log.txt") or die("error:$!");
				print LOG "${file2}_${_}_stripped.fq  > 0 reads\n";
				close(LOG);
			}
		}
	}elsif($filenum == 1){
